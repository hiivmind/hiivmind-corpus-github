# Contributing

Contributing to open source on GitHub.

> **45 files** | Source: `docs:contributing/`

---

## Overview

- **Contributing Overview** `docs:contributing/index.md`
- **Getting Started** `docs:contributing/getting-started-with-contributing.md`

## Finding Projects

- **Finding Projects** `docs:contributing/finding-ways-to-contribute-to-open-source.md`

## Making Contributions

- **Collaborating** `docs:contributing/collaborating-with-maintainers/index.md`
- **Opening Issues** `docs:contributing/collaborating-with-maintainers/opening-an-issue-from-code.md`

## Being a Maintainer

- **Maintainer Overview** `docs:contributing/being-a-good-open-source-maintainer.md`

## New in This Refresh

- **Get started content type** `docs:contributing/style-guide-and-content-model/get-started-content-type.md` — Get started content provides the minimal essential information to use a product or feature.
- **How-to content type** `docs:contributing/style-guide-and-content-model/how-to-content-type.md` — Explain how to complete a task using Github products and features.
